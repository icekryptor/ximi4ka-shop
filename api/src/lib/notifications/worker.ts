...
const BATCH_SIZE = 20
// Не больше стольких доставок на канал за тик (тик — 10 с), то есть около 18
// в минуту. Telegram пускает в группу около 20 сообщений в минуту; Google —
// около 60 запросов в минуту, а upsert строки — 2–3 запроса (~54 в минуту).
// Остальное ждёт следующего тика: накопленная очередь уходит постепенно, без 429.
