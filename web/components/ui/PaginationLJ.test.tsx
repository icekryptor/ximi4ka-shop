...
describe('<PaginationLJ>', () => {
  ...
  it('renders nothing when totalPages <= 1', () => {
    const { container } = render(<PaginationLJ {...baseProps} currentPage={1} totalPages={1} />)
  ...
  it('compresses to first + last + 3-around-current with two ellipses for many pages', () => {
    const { container } = render(<PaginationLJ {...baseProps} currentPage={6} totalPages={12} />)
  ...
  it('renders НАЗАД as a span (disabled) on page 1', () => {
    const { container } = render(<PaginationLJ {...baseProps} currentPage={1} totalPages={5} />)
  ...
  it('renders ВПЕРЁД as a span (disabled) on last page', () => {
    const { container } = render(<PaginationLJ {...baseProps} currentPage={5} totalPages={5} />)
  ...
  it('renders НАЗАД as a Link with prev page href when not on page 1', () => {
    const { container } = render(<PaginationLJ {...baseProps} currentPage={3} totalPages={5} />)
  ...
  it('renders the mono caption with correct range', () => {
    ...
    expect(
      screen.getByText(/стр\.\s*03\s*из\s*09\s*·\s*показано\s*25–36\s*из\s*100/i),
    ).toBeInTheDocument()
  })
  ...
  it('caption clamps to totalResults on the last page', () => {
    ...
    expect(
      screen.getByText(/стр\.\s*05\s*из\s*05\s*·\s*показано\s*49–50\s*из\s*50/i),
    ).toBeInTheDocument()
  })
