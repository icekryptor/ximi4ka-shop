...
describe('<CategoryTileLJ>', () => {
  ...
  it('renders mono product count with correct Russian pluralization', () => {
    ...
    const { rerender } = render(<CategoryTileLJ category={cat} productCount={1} />)
    expect(screen.getByText(/1 товар →/)).toBeInTheDocument()

    // 3 → "3 товара" (2nd form: 2-4)
    rerender(<CategoryTileLJ category={cat} productCount={3} />)
  ...
  it('renders as a bright gradient container with large radius (v3.5)', () => {
    const { container } = render(<CategoryTileLJ category={cat} productCount={42} />)
  ...
  it('falls back to the gradient + SVG molecule for a category without art', () => {
    ...
    const { container } = render(<CategoryTileLJ category={plain} productCount={42} />)
  ...
  it.each([
    ...
    const { container } = render(<CategoryTileLJ category={withArt} productCount={42} />)
  ...
  it('keeps the name readable over the photo', () => {
    ...
    render(<CategoryTileLJ category={withArt} productCount={42} />)
