...
// и та же (`<div>`), меняется только её первый ребёнок — список пунктов или
// статус загрузки. Так текст ошибки (например, «сервер не нашёл пункт») не
// мигает и не пропадает на экране, пока список после отказа перезагружается:
// это тот же узел, а не новый на месте старого.
function PickupList({
  delivery: d,
  error,
}: {
