...
async function fetchPage(id: string): Promise<Page | null> {
  ...
  const res = await fetch(`${ADMIN_API_URL_SERVER}/api/admin/pages/${encodeURIComponent(id)}`, {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-brand-text">Редактирование: {page.title}</h1>
        <Link href="/admin/pages" className="text-sm text-brand-text-secondary hover:underline">
          ← К списку
        </Link>
      </div>
